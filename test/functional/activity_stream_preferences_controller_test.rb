...
class ActivityStreamPreferencesControllerTest < ActionController::TestCase
  ...
  def test_all_checked_should_save_no_data
    ...
    post :create, :user_id => users(:aaron).id, :locations => locations
    assert_equal(0, ActivityStreamPreference.find(:all, :conditions => { :user_id => users(:aaron)}).size)
  end
  ...
  def test_view_should_contain_atom_url
    ...
    a = "http://test.host/feeds/your_activities/#{users(:aaron).activity_stream_token}"
